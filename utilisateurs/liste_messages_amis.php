<?php
	...
	while($donnees = $req->fetch()) {
		...
		if($donnees['Utilisateur_pseudo'] != $pseudo) {
			...
			$req2 = $bdd->prepare('SELECT * FROM `message` WHERE ((`Utilisateur_pseudo` = ? OR `Utilisateur_pseudo` = ?) AND `destinataire` = ?) ORDER BY date_de_creation DESC LIMIT 30');
			$req2->execute(array($donnees['Utilisateur_pseudo'], $pseudo, 'Amis'));
			$x = 0;
			...
			$req2 = $bdd->prepare('SELECT * FROM `message` WHERE ((`Utilisateur_pseudo` = ? OR `Utilisateur_pseudo` = ?) AND `destinataire` = ?) ORDER BY date_de_creation DESC LIMIT 30');
			$req2->execute(array($donnees['pseudo_ami'], $pseudo, 'Amis'));
			$x = 0;
			while($donnees2 = $req2->fetch()) {
