<?php
	...
	if($donnees = $req->fetch()) {
		...
		*/
		$req6 = $bdd->prepare('SELECT ( SELECT `Utilisateur_pseudo`, ( COUNT(`Utilisateur_pseudo`) AS `nb_message` FROM `message` GROUP BY `Utilisateur_pseudo` ORDER BY `nb_message`) FROM `message` WHERE `Utilisateur_pseudo` = ?');
