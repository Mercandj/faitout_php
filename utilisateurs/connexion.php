<?php
	...
	if($donnees = $req->fetch()) {
		...
		$req6 = $bdd->prepare('SELECT row_number() AS `rang`, `Utilisateur_pseudo`, COUNT(*) AS `nb_message` FROM `message` WHERE `Utilisateur_pseudo` = ? GROUP BY `Utilisateur_pseudo` ORDER BY `nb_message`');
