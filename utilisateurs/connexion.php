<?php
	...
	if($donnees = $req->fetch()) {
		...
		*/
		$req6 = $bdd->prepare('SELECT * FROM ( SELECT * FROM ( SELECT `Utilisateur_pseudo`, ( COUNT(`Utilisateur_pseudo`) AS `nb_message` ) FROM `message` ORDER BY `nb_message` ) GROUP BY `Utilisateur_pseudo`) WHERE `Utilisateur_pseudo` = ?');
